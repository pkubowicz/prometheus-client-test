...
public class Main {
  ...
  public static void main(String[] args) throws IOException {
    ...
    for (int i = 0; i < ITERATIONS; i++) {
      runPerformant2TextFormat();
    }
    throwawayTimer.close();

    // test new 'performant2' code
    runPerformantTextFormat();
    Gauge perf2Gauge = new Gauge.Builder().name("performant2").help("time performant2 code").create();
    Timer perf2Timer = perf2Gauge.startTimer();
    for (int i = 0; i < ITERATIONS; i++) {
      runPerformant2TextFormat();
    }
    perf2Timer.close();
    printStats(perf2Gauge.collect());
  ...
  static void runPerformant2TextFormat() throws IOException {
    StringWriter sw = new StringWriter(10240);
    PerformantTextFormat2.write004(sw, CollectorRegistry.defaultRegistry.metricFamilySamples());
    sw.close();
  }
